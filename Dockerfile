...
COPY ./soft/libevent-2.1.8-stable.tar.gz /usr/src/libevent-2.1.8-stable.tar.gz   
COPY ./soft/libiconv-1.15.tar.gz /usr/src/libiconv-1.15.tar.gz   
...
RUN sed -i 's/dl-cdn.alpinelinux.org/mirrors.aliyun.com/g' /etc/apk/repositories \      
        ...
        && tar -xvf redis-$PHP_REDIS.tgz && rm -rf redis-$PHP_REDIS.tgz && cd redis-$PHP_REDIS && $PHP_DIR/bin/phpize && ./configure --with-php-config=$PHP_DIR/bin/php-config && make && make install && make clean && cd .. \
        ...
        && tar -xvf protobuf-$PHP_PROTOBUF.tgz && rm -rf protobuf-$PHP_PROTOBUF.tgz && cd protobuf-$PHP_PROTOBUF && $PHP_DIR/bin/phpize && ./configure --with-php-config=$PHP_DIR/bin/php-config && make && make install && make clean && cd .. \
        ...
        # &&  wget http://pecl.php.net/get/event-2.3.0.tgz && tar -zxvf event-2.3.0.tgz && rm -rf  event-2.3.0.tgz && cd event-2.3.0 && $PHP_DIR/bin/phpize && ./configure --with-php-config=$PHP_DIR/bin/php-config --with-event-libevent-dir=/usr/local/libevent-2.1.8/ && make && make install && make clean && cd .. \
        &&   tar -zxvf event-2.3.0.tgz && rm -rf  event-2.3.0.tgz && cd event-2.3.0 && $PHP_DIR/bin/phpize && ./configure --with-php-config=$PHP_DIR/bin/php-config --with-event-libevent-dir=/usr/local/libevent-2.1.8/ && make && make install && make clean && cd .. \
        &&  { echo 'extension = event.so'; } | tee $PHP_DIR/etc/php.d/event.ini \         
        && { \
        echo '[global]'; \
